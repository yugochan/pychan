...
# データ読み込み
df = pd.read_csv("", encoding="utf-8", skiprows=0, header=1)
df['日付'] = pd.to_datetime(df['日付'], format = '%Y-%m-%d')

#データの標準化
df['終値'] = preprocessing.scale(df['終値'])
df = df.loc[:, ['日付', '終値']]
...
for i in range(len(df) - maxlen):
    X.append(df[['終値']].iloc[i:(i+maxlen)].as_matrix())
    Y.append(df[['終値']].iloc[i+maxlen].as_matrix())
X=np.array(X)
Y=np.array(Y)
